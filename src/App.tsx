...
## 维度二：岗位硬技能考察（2-5题）
根据JD中明确要求的工具/技能，识别技能类型后选择对应的出题形式：

### 识别规则（按简历+JD判断）：
- 出现 SQL / 数据库 / 数仓 → 出 SQL 手撕题
- 出现 Python / pandas / numpy / 数据处理 → 出 Python 代码题
- 出现 Excel / 数据透视表 / 函数 → 出 Excel 操作题
- 出现 Tableau / Power BI / 数据可视化 → 出可视化方案设计题
- 出现 A/B测试 / 统计学 / 假设检验 → 出统计推断题
- 出现 产品设计 / PRD / 原型 → 出产品方案设计题
- 出现 运营策略 / 增长 / 活动策划 → 出业务场景分析题
- 出现 Prompt工程 / AI工具 / LLM → 出 Prompt 设计/评估题
- 出现 机器学习 / 模型 / 算法 → 出算法思路题
- 出现 项目管理 / 需求排期 → 出优先级判断/资源分配题

### 各技能出题形式（必须贴近真实面试场景）：

**SQL题**（最常见，适合数据/分析/后端岗）：
- 给出具体表结构（2-3张表，字段名用中文业务名命名），结合JD业务场景出题
- 题型：①业务场景现写（最常见）②窗口函数应用（留存/排名/连续登录）③多表JOIN查询 ④SQL优化题
- 例：「用户表(user_id, reg_date)，订单表(order_id, user_id, amount, order_date)，写SQL求近30天每日新用户的首单转化率」
- 难度：中等，考察思路+语法，不超过30行代码

**Python题**（适合数据/算法/开发岗）：
- 给出数据处理场景或算法逻辑，考察 pandas/numpy/基础算法
- 题型：①数据清洗处理 ②统计计算 ③逻辑实现题（不超过15行）
- 例：「有一个DataFrame，含user_id/event_time/event_type，用pandas找出连续3天活跃的用户」

**Excel题**（适合运营/产品/商分岗）：
- 描述一个业务数据场景，问如何用函数/数据透视表解决
- 题型：①VLOOKUP/INDEX-MATCH场景应用 ②数据透视表设计 ③条件统计公式
- 例：「有两张表，一张是订单明细，一张是产品信息，如何快速匹配并统计各品类GMV？」

**统计/A-B测试题**（适合数据分析/增长岗）：
- 给出实验数据，问如何判断结果是否显著、如何设计实验
- 例：「实验组转化率3.2%（n=5000），对照组2.8%（n=5000），这个实验结论可信吗？」

**产品/业务场景题**（适合产品/运营岗）：
- 给出一个具体业务指标下降/功能设计需求，考察分析框架
- 形式：口头描述方案，不需要写代码，考察结构化思维

**Prompt/AI工具题**（适合AIGC/AI产品岗）：
- 给出一个具体AI应用场景，考察 Prompt 设计能力
- 例：「设计一个 Prompt，让 LLM 稳定输出符合格式要求的JSON」

### 出题原则：
- 每道题必须结合JD中的实际业务背景，不出泛泛而谈的通用题
- SQL/Python题必须给出完整的表结构或数据样例，让候选人知道从哪入手
- 难度适中：考察思路+基础语法，不出需要记忆冷门API的题
- 每道题附带标准答案要点（answer_points字段）
...

{
  "sections": [
          ...
          "questions": [
            {
              ...
              "answer_points": ["优秀回答应包含的要点1", "要点2", "要点3"]
            }
          ]
        }
      ]
    },
    {
      ...
      "questions": [
        {
          "q": "题目描述（包含完整的表结构/数据样例/业务背景，让候选人清楚题目条件）",
          "skill": "技能类型（SQL / Python / Excel / 统计 / 产品设计 / 业务分析 / Prompt设计）",
          "format": "题目形式（业务场景现写 / 窗口函数 / 数据处理 / 实验设计 / 方案设计）",
          "difficulty": "难度（初级 / 中级 / 进阶）",
          "code_template": "代码填空模板或起始框架（仅SQL/Python题需要，其他留空字符串）",
          "key_points": ["考察要点1", "考察要点2"],
          "answer_points": ["答案要点1", "答案要点2", "答案要点3"],
          "intent": "考察意图"
        }
      ]
    },
...
function LandingPage({ onEnter }: { onEnter: () => void }) {
  ...
  const features = [
    {
      icon: "📊",
      title: "智能匹配分析",
      desc: "AI 从招聘决策视角深度解析简历与 JD 的匹配度，给出 0–100 精准评分，清晰识别核心差距。",
      color: "#3b82f6",
      tag: "",
    },
    {
      icon: "💡",
      title: "针对性优化建议",
      desc: "5 条结构化建议，精准定位到简历的具体位置，告诉你改什么、怎么改、改完有什么效果。",
      color: "#8b5cf6",
      tag: "",
    },
    {
    ...
    },
    {
      icon: "🎯",
      title: "深度面试准备",
      desc: "基于你的简历与 JD 生成专属面试题——简历追问链、硬技能手撕题（SQL/Python/Excel 等）、能力差距探测，每题附参考答案。",
      color: "#f59e0b",
      tag: "NEW",
    },
    {
  ...
  const steps = [
    { num: "01", title: "上传简历 + 粘贴 JD", desc: "支持 PDF / 图片导入，AI 自动识别并结构化内容" },
    { num: "02", title: "获取匹配评分", desc: "多维度评分，清晰看到优势亮点与能力差距" },
    { num: "03", title: "AI 优化改写", desc: "生成 5 条优化建议，一键改写简历强化关键词匹配" },
    { num: "04", title: "生成专属面试题", desc: "基于简历 + JD 深度定制面试题，含追问链与参考答案" },
    { num: "05", title: "导出投递", desc: "在线调整排版，导出 PDF 直接投递目标岗位" },
  ];

  const faqs = [
    { q: "我的简历数据安全吗？", a: "所有数据仅在你的浏览器本地处理，历史记录保存在本地，不会上传到任何服务器。" },
    { q: "面试题是通用题库吗？", a: "不是。面试题完全基于你的简历经历和目标 JD 深度定制生成，包含对你项目的层层追问、岗位技能手撕题（SQL/Python/Excel 等），以及针对你简历短板的探测题。" },
    { q: "支持哪些简历格式？", a: "支持 PDF、PNG、JPG、WEBP 等格式上传，也可以直接粘贴文字。AI 会自动识别并整理结构。" },
    { q: "适合哪些人使用？", a: "适合任何正在求职的人，尤其是需要针对不同岗位定制简历、备战面试的求职者。" },
  ...
  return (
    <div style={{
          ...
          {features.map((f, i) => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${hovered === "feat-" + i ? f.color + "55" : "rgba(255,255,255,0.06)"}`,
                borderRadius: 16, padding: "28px 24px",
                transition: "all 0.25s ease",
                cursor: "default",
                transform: hovered === "feat-" + i ? "translateY(-4px)" : "translateY(0)",
                boxShadow: hovered === "feat-" + i ? `0 8px 32px ${f.color}20` : "none",
      ...
      <section style={{
        ...
        <p style={{ color: "#64748b", fontSize: 16, marginBottom: 40 }}>免费使用 · 无需注册 · 简历优化 + 面试准备一站搞定</p>
        <button
          onClick={onEnter}
...
export default function App() {
  ...
  const toggleAnswer = (key: string) => setShowAnswers(prev => ({ ...prev, [key]: !prev[key] }));
  ...
  const handlePdfUpload = async (file: File) => {
    ...
    // 图片格式：发到后端 /api/ocr → Groq Llama 4 视觉模型识别文字并直接结构化
    if (isImg) {
      ...
      try {
        ...
        // Step 2: 发到后端 /api/ocr，由 Groq 视觉模型识别 + 结构化（一步到位）
        const structurePrompt = fillPrompt(prompts.structurePdf, { rawText: "[图片简历，请直接从图片中识别所有文字并按下方要求结构化输出]" });
        ...
        if (!ocrRes.ok) throw new Error(`图片识别请求失败 (${ocrRes.status})`);
        const ocrData = await ocrRes.json();
        if (ocrData.error) throw new Error(ocrData.error.message || "识别失败");
        const structured = ocrData.choices?.[0]?.message?.content?.trim();
        if (!structured) throw new Error("模型未返回内容，请重试");
              ...
              <div>
                ...
                {interviewResult.sections.map((section, si) => (
                  <div key={si} style={{ marginBottom: 24 }}>
                            ...
                            <div style={{ padding: "4px 18px 16px" }}>
                              {group.questions.map((q, qi) => (
                                <div key={qi} style={{ marginTop: 14, paddingTop: 14, borderTop: qi > 0 ? "1px solid #1e293b" : "none" }}>
                                  ...
                                  <div style={{ marginLeft: 32, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                                    <div style={{ display: "inline-flex", gap: 6, alignItems: "center", background: "#1e293b55", padding: "4px 10px", borderRadius: 6 }}>
                                      <span style={{ fontSize: 10, color: "#475569" }}>考察：</span>
                                      <span style={{ fontSize: 11, color: "#64748b" }}>{q.intent}</span>
                                    </div>
                                    {q.answer_points && q.answer_points.length > 0 && (() => {
                                      const rKey = `r-${si}-${gi}-${qi}`;
                                      const isAns = showAnswers[rKey];
                                      return (
                                        <button onClick={() => toggleAnswer(rKey)} style={{ background: isAns ? "rgba(59,130,246,0.12)" : "#1e293b", border: `1px solid ${isAns ? "rgba(59,130,246,0.3)" : "#334155"}`, borderRadius: 6, padding: "4px 10px", color: isAns ? "#60a5fa" : "#64748b", fontSize: 11, cursor: "pointer", fontWeight: 600, flexShrink: 0 }}>
                                          {isAns ? "▲ 收起答案" : "✦ 参考答案"}
                                        </button>
                                      );
                                    })()}
                                  </div>
                                  {q.answer_points && q.answer_points.length > 0 && showAnswers[`r-${si}-${gi}-${qi}`] && (
                                    <div style={{ marginLeft: 32, marginTop: 10, padding: "12px 16px", background: "rgba(59,130,246,0.04)", border: "1px solid rgba(59,130,246,0.1)", borderRadius: 10 }}>
                                      <div style={{ fontSize: 12, color: "#475569", marginBottom: 8 }}>优秀回答应包含：</div>
                                      {q.answer_points.map((pt, pi) => (
                                        <div key={pi} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                                          <span style={{ flexShrink: 0, color: "#3b82f6", fontSize: 12 }}>·</span>
                                          <span style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>{pt}</span>
                                        </div>
                                      ))}
                                    </div>
                                  )}
                    ...
                    {/* 其他维度：flat 列表（含 AI 答案） */}
                    {section.type !== "resume_deep_dive" && section.questions?.map((q, qi) => {
                      const aKey = `${si}-flat-${qi}`;
                      const isAnswerOpen = showAnswers[aKey];
                      const accentColor = section.type === "hard_skills" ? "#f97316" : section.type === "gap_probe" ? "#818cf8" : "#34d399";
                      const accentBg = section.type === "hard_skills" ? "rgba(249,115,22,0.08)" : section.type === "gap_probe" ? "rgba(129,140,248,0.08)" : "rgba(52,211,153,0.08)";
                      const accentBorder = section.type === "hard_skills" ? "rgba(249,115,22,0.2)" : section.type === "gap_probe" ? "rgba(129,140,248,0.2)" : "rgba(52,211,153,0.2)";
                      return (
                        <div key={qi} style={{ marginBottom: 16, background: "#080d1a", border: `1px solid ${accentBorder}`, borderLeft: `3px solid ${accentColor}`, borderRadius: 14, overflow: "hidden" }}>
                          {/* 题目头部 */}
                          <div style={{ padding: "20px 22px 16px" }}>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
                              <span style={{ flexShrink: 0, width: 26, height: 26, borderRadius: "50%", background: accentBg, border: `1px solid ${accentBorder}`, color: accentColor, fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center" }}>Q{qi + 1}</span>
                              <div style={{ flex: 1 }}>
                                <div style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0", lineHeight: 1.7, marginBottom: 12 }}>{q.q}</div>
                                {/* 标签行 */}
                                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const, marginBottom: 4 }}>
                                  {q.skill && <span style={{ fontSize: 11, background: "rgba(249,115,22,0.12)", color: "#fb923c", padding: "3px 9px", borderRadius: 99, border: "1px solid rgba(249,115,22,0.25)", fontWeight: 600 }}>🔧 {q.skill}</span>}
                                  {q.format && <span style={{ fontSize: 11, background: "#1e293b", color: "#94a3b8", padding: "3px 9px", borderRadius: 99 }}>{q.format}</span>}
                                  {q.difficulty && <span style={{ fontSize: 11, background: q.difficulty === "初级" ? "rgba(52,211,153,0.1)" : q.difficulty === "中级" ? "rgba(250,204,21,0.1)" : "rgba(248,113,113,0.1)", color: q.difficulty === "初级" ? "#34d399" : q.difficulty === "中级" ? "#facc15" : "#f87171", padding: "3px 9px", borderRadius: 99, fontWeight: 600 }}>{q.difficulty}</span>}
                                  {q.gap && <span style={{ fontSize: 11, background: "rgba(129,140,248,0.1)", color: "#a5b4fc", padding: "3px 9px", borderRadius: 99, border: "1px solid rgba(129,140,248,0.2)" }}>差距点：{q.gap}</span>}
                                  {q.category && <span style={{ fontSize: 11, background: "rgba(52,211,153,0.1)", color: "#6ee7b7", padding: "3px 9px", borderRadius: 99 }}>{q.category}</span>}
                                </div>
                              </div>
                            </div>

                            {/* 代码模板（SQL/Python题） */}
                            {q.code_template && q.code_template.trim() && (
                              <div style={{ margin: "0 0 14px 38px", background: "#0a0f1e", border: "1px solid #1e293b", borderRadius: 10, overflow: "hidden" }}>
                                <div style={{ padding: "8px 14px", background: "#0f172a", borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: 6 }}>
                                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#f97316", display: "inline-block" }} />
                                  <span style={{ fontSize: 11, color: "#64748b", fontFamily: "monospace" }}>代码框架</span>
                                </div>
                                <pre style={{ margin: 0, padding: "14px 16px", fontSize: 12, color: "#94a3b8", fontFamily: "monospace", lineHeight: 1.7, overflowX: "auto" as const, whiteSpace: "pre-wrap" as const }}>{q.code_template}</pre>
                              </div>
                            )}

                            {/* 考察意图 */}
                            <div style={{ marginLeft: 38, fontSize: 12, color: "#475569", lineHeight: 1.6 }}>
                              <span style={{ color: "#334155" }}>考察意图：</span>{q.intent}
                            </div>
                          </div>
                          ...
                          <div style={{ borderTop: "1px solid #0f172a" }}>
                            <button
                              ...
                              style={{ width: "100%", padding: "12px 22px", background: isAnswerOpen ? "rgba(99,102,241,0.08)" : "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "background 0.2s" }}
                            >
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ fontSize: 13 }}>✦</span>
                                <span style={{ fontSize: 13, fontWeight: 600, color: isAnswerOpen ? "#818cf8" : "#475569" }}>查看参考答案</span>
                              </div>
                              <span style={{ fontSize: 11, color: "#334155" }}>{isAnswerOpen ? "▲ 收起" : "▼ 展开"}</span>
                            </button>
                            {isAnswerOpen && (
                              <div style={{ padding: "4px 22px 20px", background: "rgba(99,102,241,0.04)", borderTop: "1px solid rgba(99,102,241,0.1)" }}>
                                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 10, paddingTop: 12 }}>答案要点（基于你的简历背景）：</div>
                                {q.answer_points && q.answer_points.length > 0 ? (
                                  <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                                    {q.answer_points.map((pt, pi) => (
                                      <div key={pi} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                                        <span style={{ flexShrink: 0, width: 18, height: 18, borderRadius: "50%", background: "rgba(99,102,241,0.15)", color: "#818cf8", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2 }}>{pi + 1}</span>
                                        <span style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.7 }}>{pt}</span>
                                      </div>
                                    ))}
                                  </div>
                                ) : (
                                  <div style={{ fontSize: 13, color: "#475569" }}>暂无答案要点</div>
                                )}
                              </div>
                            )}
                          </div>
